...
function testArgList(list, content_type, lineNo, line) {
    let errors = [];
...
function testArgInt(number, lineNo, line) {
    let errors = [];
...
function testArgStr(str, lineNo, line) {
    let errors = [];
    let regex = /".+(?<![^\\]\\)"/;
    if (!regex.test(str)) {
...
function testArgRegex(regex, lineNo, line) {
    let errors = [];
    if (!testRegex(regex)) {
...
function diagConnectionLib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagCryptLib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagFileLib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagIpv4Lib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagListLib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagSysLib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagStringLib(func, args, lineNo, line) {
    let errors = [];
    let minargs = 0;
    let maxargs = 0;
    switch (func) {
...
function diagCodeLine(trimmed, lineNo, line) {
    let errors = [];
    ...
    let varRe = new RegExp(/(\w+) ?= ?/ + '|' + exp2Re.source);
    ...
    if (match && match.length == 3) {
        if (builtin.test(match[1])) {
            ...
            exports.variables[match[1]] = match[2];
        }
        return errors;
    }
    //control keywords
    if (keywords.test(trimmed)) {
        ...
        switch (args[0]) {
            case "append":
                ...
                else if (!exports.variables[args[1]]) {
                    errors.push(createDiagError('Variable not declared.', range1));
                }
                else if (!exports.variables[args[3]]) {
